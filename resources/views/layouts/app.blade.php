...
<head>
    ...
    <title>{{ config('app.name', 'Connect ISEST') }}</title>
...
<body>
    <div id="app">
        <nav class="bg-white navbar sticky-top navbar-expand-md navbar-light border-btm-e6">
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <ul class="navbar-nav">
                        <li class="nav-item">
                            @if (session()->has('browse_session_name') && session('browse_session_name') !==
                            $current_school_session_name)
                            <a class="nav-link text-danger disabled" href="#" tabindex="-1" aria-disabled="true">
                                <i class="bi bi-exclamation-diamond-fill me-2"></i>
                                {{ __('Browsing as Academic Session') }} {{ session('browse_session_name') }}
                            </a>
                            @elseif(\App\Models\SchoolSession::latest()->count() > 0)
                            <a class="nav-link disabled" href="#" tabindex="-1" aria-disabled="true">
                                {{ __('Current Academic Session') }} {{ $current_school_session_name }}
                            </a>
                            @else
                            <a class="nav-link text-danger disabled" href="#" tabindex="-1" aria-disabled="true">
                                <i class="bi bi-exclamation-diamond-fill me-2"></i>
                                {{ __('Create an Academic Session.') }}
                            </a>
                            @endif
                        </li>
                    </ul>
